...
## 이식 메모 (Semi-AD, 2026-08-23)

`Semi-AD-OOD/src/a27_line_filter.py` 에서 옮겨 왔다(그쪽 worktree 는 읽기만 했다).
OOD 전용인 반경 마스크 arm 은 가져오지 않았다 — 여기서는 지도학습 진단에만 쓴다.

여기서 쓰는 이유는 E20 진단에서 나왔다. **Scratch 의 국소 밀도 중앙값 0.500 은
none 의 0.400 다음으로 낮다.** 가는 선은 정사각 창을 원리적으로 채우지 못한다.
...
def line_density_map(x, length: int = 7, n_orient: int = 8, width: int = 1,
                     valid=None, chunk: int = 2000, min_dies: int = 0) -> np.ndarray:
    """다이별 **방향 최대 선 밀도** 맵. 창 안 다이 개수로 나눈다.

    `valid` 를 주면 그 마스크의 다이만 분모와 분자에 넣는다
    (중심 다이 제외 arm 에 쓴다).

    `min_dies` 는 **창이 웨이퍼 가장자리에서 잘렸을 때를 버린다.**
    2026-08-23 진단에서 이게 필요한 이유가 드러났다 — 다이 개수로 나누므로
    가장자리에서 창이 3칸으로 줄면 **연속 3개만 불량이어도 1.0** 이 된다.
    정상 웨이퍼도 불량 다이가 10% 쯤 무작위로 있으므로 그런 3연속이 우연히 생기고,
    실제로 **참 none 의 71.1% 가 1.0 으로 포화했다**(포화 칸의 창 안 다이 최빈값 3).
    `min_dies=length` 로 두면 온전한 창만 본다. 기본 0 은 이식 원본과 같은 동작이다.
    """
    x = np.asarray(x)
...
def line_density_stack_torch(x, ls=(7,), n_orient: int = 8, width: int = 1):
    """(N,H,W) 정수 텐서 -> (N,len(ls),H,W). 길이마다 `min_dies=length` 를 건다.

    **정의는 `line_density_map` 이고 이건 속도용 사본이다.** 8방향 커널을 그대로
    conv2d 필터 뱅크로 놓고 방향 축에서 최대를 취한다. 커널이 중심 대칭이라
    상관과 합성곱이 같으므로 뒤집기가 필요 없다.
    두 구현이 같은 값을 낸다는 것을 `tests/test_a22_line_filter.py` 가 대조한다.
    """
    import torch
    ...
    if x.ndim == 2:
        x = x[None]
    if not len(ls):
        raise ValueError("길이를 하나 이상 줘야 한다")
    die = (x > 0).float().unsqueeze(1)
    fail = (x == 2).float().unsqueeze(1)
    out = []
    for L in ls:
        key = (L, n_orient, width)
        if key not in _KERNEL_CACHE:
            k = np.stack(line_kernels(L, n_orient, width)).astype(np.float32)
            _KERNEL_CACHE[key] = torch.from_numpy(k).unsqueeze(1)
        w = _KERNEL_CACHE[key].to(die.device, die.dtype)
        num = F.conv2d(fail, w, padding=L // 2)
        den = F.conv2d(die, w, padding=L // 2)
        r = torch.where(den >= L, num / den.clamp_min(1e-12), torch.zeros_like(num))
        out.append(torch.where(die > 0, r.max(1, keepdim=True).values,
                               torch.zeros_like(die)))
