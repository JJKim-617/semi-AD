...
- `specs/` — **산출물 사양**. 파이프라인이 만들어낸 것이 무엇인지 기술한다
  (`data_cache.md` 전처리 캐시, `splits.md` 분할). 가설 검증이 아니라 안정적인 사실을 담는다.
  전처리나 분할처럼 **실험이 아닌 파이프라인 단계는 `experiments/` 가 아니라 여기**에 둔다.
- `plans/` 계획 · `adr/` 설계 결정 기록
- `experiments/` — **가설을 검증한 것만**. 요약과 상세를 분리한다.
  - 최상위: 실험군 요약 md. 진입점이며 실험 목록과 결론만 담고 상세는 링크한다.
  - `candidate/`: 가설. 무엇을 검증하는지, **무엇이면 기각인지**를 실험 전에 적는다.
  - `details/`: 개별 실행 1건 = md 1개. 조건, 결과, 학습 추이, 재현 명령.
